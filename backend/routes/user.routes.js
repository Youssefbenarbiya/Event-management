...
userRouter.post("/register", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: newUser._id }, process.env.token, {
      expiresIn: "24h",
    });
...
userRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!userPresent) {
      return res
        .status(401)
        .json({ error: "User is not registered. Please register first." });
    }
...
userRouter.put("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "User update failed. Please try again later." });
  }
});
