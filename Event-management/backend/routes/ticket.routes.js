...
ticketRouter.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Ticket booking failed. Please try again later." });
  }
});
...
ticketRouter.get("/user", authMiddleware, async (req, res) => {
  try {
    ...
    const userTickets = await Ticket.find({ user: req.userId }).populate(
      "event"
    );
    ...
    res
      .status(500)
      .json({ error: "Ticket retrieval failed. Please try again later." });
  }
});
...
ticketRouter.get("/event/:eventId", authMiddleware, async (req, res) => {
  ...
  try {
    // Retrieve tickets associated with the event
    const eventTickets = await Ticket.find({ event: eventId }).populate(
      "event"
    );
    res.status(200).json({ tickets: eventTickets });
  } catch (err) {
    console.error(err.message);
    res
      .status(500)
...
ticketRouter.delete("/:ticketId", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Ticket cancellation failed. Please try again later." });
  }
});
